...
window.addEventListener("load", () => {
    ...
    const ocupadas = localStorage.getItem("teatroOcupadas") // poltronas
        ? localStorage.getItem("teatroOcupadas").split(";")
        : [];

        // repetiçao para montar o n total de poltronas
        for (let i = 1; i <= POLTRONAS; i++) {
            const figure = document.createElement("figure"); // cria tag figure
            const imgStatus = document.createElement("img"); // cria tag img

            // se a posição consta em ocupadas, exibe a imagem ocupada, senão, disponivel
        ...
        // se i modulo 24 == 0: o comando após && será executado (insere quebre de linha)
...
frm.addEventListener("submit", (e) => {
    ...
    const poltrona = Number(frm.inPoltrona.value); // obtem

    // valida o preenchimento do campo de entrada... nao pode ser maior que a const
...
frm.btConfirmar.addEventListener("click", () => {
    ...
    for (let i = reservadas.length - 1; i >= 0; i--) {
        ...
        // captura img da poltrona, filha de divPalco. é -1 pois começa em 0
